...
#   ">", ">=", "<", "<=", "==", "!=", "&", "|", "!", xor(x,y)
# NOTE: There is a difference between "&&" and "&". Using & on logical vectors will result in a vector. Using only & will return
#   the result of the comparison of the first element of both vectors. Make sure to && when dealing with numerics or with 
#   vectors of length one. 
# the same thing as above applies to "||" and "|"
...
# Modifying data frames is the same as modifying matricies, we simple use both te selection and assignment operators

# We can delete data frame columns by assigning them the value of NULL
...
# Factors can also be modified using a combination of the selection and assignment operators. However, we cannot assign values
#   outside of the defined levels.

# If we would like to add a value outside of predefined levels, then we must add a new level first using
#   levels(factor) <- c(levels(factor), "new_level")
